...
You also should make the spell checker ignore punctuation. In particular, `spellcheck.py``` should be able to handle a trailing ,, ., ?, or ! or ;. For instance, it should be able to change `hudnred?` to `hundred?` in replace mode. How can this be accomplished?
